...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .header { background: linear-gradient(135deg, #1c1917, #064e3b); padding: 24px 40px; display: flex; align-items: center; gap: 16px; }
    ...
    #login-section input:focus { border-color: #065f46; box-shadow: 0 0 0 3px rgba(6,95,70,.15); }
    ...
    .search-bar:focus { border-color: #065f46; box-shadow: 0 0 0 3px rgba(6,95,70,.15); }
    ...
    input[type=text]:focus, textarea:focus { border-color: #065f46; box-shadow: 0 0 0 3px rgba(6,95,70,.15); }
    textarea { min-height: 160px; resize: vertical; }
    .field { margin-bottom: 16px; }
    .hint { font-size: 12px; color: #9ca3af; margin-top: 4px; }

    button { background: #065f46; color: #fff; border: none; border-radius: 8px; padding: 10px 22px; font-size: 14px; font-weight: 600; cursor: pointer; transition: background .15s; }
  ...
  <div id="panel-section">
    ...
    <div id="tab-send" class="section">
      <div class="card">
        ...
        <p style="font-size:13px;color:#6b7280;margin:0 0 20px;">Wird an alle <strong>bestätigten</strong> Abonnenten verschickt. Jede E-Mail erhält automatisch einen persönlichen Abmeldelink.</p>
        <div class="field">
          <label for="subject">Betreff</label>
          <input type="text" id="subject" placeholder="z.B. Neuigkeiten vom FoodConnectMarkt" />
        </div>
        <div class="field">
          <label for="body-html">Inhalt (HTML)</label>
          <textarea id="body-html" placeholder="<p>Hallo,</p><p>...</p>"></textarea>
          <p class="hint">HTML-formatierter E-Mail-Inhalt. Ein persönlicher Abmeldelink wird automatisch am Ende eingefügt.</p>
        </div>
        <div class="field">
          <label for="body-text">Inhalt (Plaintext)</label>
          <textarea id="body-text" style="min-height:80px" placeholder="Hallo, …"></textarea>
          <p class="hint">Fallback für E-Mail-Clients ohne HTML-Unterstützung.</p>
        </div>
...
<script>
  ...
  function filterTable() {
    ...
    renderTable(q ? allRows.filter(r => r.email.toLowerCase().includes(q)) : allRows);
  }
  ...
  async function sendNewsletter() {
    ...
    if (!subject || !html || !text) {
      showMsg(msgEl, 'error', 'Bitte alle Felder ausfüllen.');
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        showMsg(msgEl, 'success', 'Gesendet an ' + data.sent + ' von ' + data.total + ' Abonnenten.' +
          (data.errors?.length ? ' ' + data.errors.length + ' Fehler.' : ''));
      } else {
        showMsg(msgEl, 'error', data.message || 'Fehler beim Senden.');
      }
    } catch (err) {
      showMsg(msgEl, 'error', 'Netzwerkfehler: ' + err.message);
    }
    btn.disabled = false;
  }
